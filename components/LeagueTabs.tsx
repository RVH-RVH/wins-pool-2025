...
export default function LeagueTabs({ leagueKey }: { leagueKey: string }) {
      ...
      {items.map((it) => {
        ...
        return (
          <Link
            key={it.href}
            href={it.href}
            className={`px-3 py-1 rounded ${active ? "bg-blue-600 text-white" : "text-blue-600 hover:bg-blue-100"}`}
          >
            {it.label}
          </Link>
        );
